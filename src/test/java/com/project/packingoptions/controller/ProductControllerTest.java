...
class ProductControllerTest {
    ...
    void testUpdateProduct() throws Exception {
        ...
        ProductRequest request = TestDataFactory.createProductRequest(product1Code, updatedName, updatedPrice);
        Product updatedProduct = TestDataFactory.createProduct(product1Code, updatedName, updatedPrice);

        when(productService.updateProduct(eq(product1Code), any(ProductRequest.class)))
    ...
    void testUpdateProductNotFound() throws Exception {
        String nonExistentCode = TestDataFactory.generateProductCode();
        ProductRequest request = TestDataFactory.createProductRequest(nonExistentCode, "Unknown",
                new BigDecimal("9.99"));

        when(productService.updateProduct(eq(nonExistentCode), any(ProductRequest.class)))
